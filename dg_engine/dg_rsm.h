...
static inline dg_canon dg_rsm_general_helper_serial_to_cert( dg_serial Serial) {
	return dg_hash_mm3_fmix64(Serial.Raw);
}

static inline dg_serial dg_rsm_general_helper_cert_to_serial( dg_canon Canon ) {
	return (dg_serial)dg_hash_mm3_fmix64_inverse(Canon);
}

//Was slot considered dead?
// For tombstoning to work mem words need to be plus 32b in size. unlikely issue nowadays but implicit contract with memory.
static inline bool dg_rsm_registry_helper_entry_is_disenrolled( dg_rsm_entry* Entry )     {
	return ( (dg_ptr) Entry & DG_RSM_DEAD_BIT ) != 0u;
}

static inline dg_rsm_entry* dg_rsm_registry_helper_entry_kill( dg_rsm_entry*   Entry )     {
	return (dg_rsm_entry*) ( (dg_ptr) Entry | DG_RSM_DEAD_BIT );
}

static inline dg_rsm_entry* dg_rsm_registry_helper_entry_resuscitate( dg_rsm_entry*   Entry )     {
	return (dg_rsm_entry*) ( (dg_ptr) Entry & ~ DG_RSM_DEAD_BIT );
}

static inline  dg_canon* dg_rsm_object_access_ptr_certs ( const dg_rsm_object*   Object )     {
	return DG_RSM_OFFSET_ACCESS(  dg_canon* , Object , CertsOffset );
}

static inline  dg_inwords32_ptrbool* dg_rsm_object_access_ptr_payload_locations ( const dg_rsm_object*   Object )     {
	return DG_RSM_OFFSET_ACCESS(  dg_inwords32_ptrbool*, Object, OffsetArrayOffset );
}

static inline  dg_byte* dg_rsm_object_access_ptr_hashtable ( const dg_rsm_object*   Object )     {
...
static inline dg_u64* dg_rsm_object_access_ptr_earlyqualfier( dg_rsm_object*   Object )     {
	return DG_RSM_OFFSET_ACCESS( dg_u64* , Object , EarlyQualifierTableOffset );
}

static inline bool dg_rsm_object_helper_index_is_disenrolled( dg_dword Phrase, dg_u8 Slot ) {
...
static inline dg_i16 dg_rsm_object_helper_retrieve_cert_index( dg_rsm_object*   Object, dg_canon Cert )     {
	if ( Object == nullptr ) {
		return -1;
	}

	dg_u8 Numeric = Object->Header.ItemCount;
	const dg_canon* Certs = dg_rsm_object_access_ptr_certs( Object );
	...
	if ( DG_RSM_PREFER_LINEAR(Numeric)) {
		...
		const dg_byte* HashTable = dg_rsm_object_access_ptr_hashtable(Object);
...
static inline dg_ptr* dg_rsm_object_helper_retrieve_at_index( dg_rsm_object* Object, dg_u8 Index) {
	if ( Object == nullptr ) {
		return nullptr;
	}
	dg_dword Removed = Object->Header.RemovedCertsIndexes;
	if ( Removed.byte[0] != DG_RSM_EMPTY_INDEX && dg_rsm_object_helper_index_is_disenrolled( Removed , Index )) {
		return nullptr;
	}
	...
	dg_inwords32_ptrbool Locale = dg_rsm_object_access_ptr_payload_locations(Object)[Index];
	dg_ptr* Slot = (dg_ptr*)( Object->Memory + DG_AS_BYTES((dg_u64)Locale.Value) );
	return Locale.IsPointer ? (dg_ptr*)*Slot : Slot;
}

static inline dg_ptr* dg_rsm_object_access_struct_via_cert( dg_rsm_object* Object, dg_canon Cert ) {
	dg_i16 Index = dg_rsm_object_helper_retrieve_cert_index(Object, Cert);
	if ( Index < 0 ) {
		return nullptr;
	}
	return dg_rsm_object_helper_retrieve_at_index( Object, (dg_u8)Index );
}

static inline dg_ptr* dg_rsm_object_access_struct_via_serial( dg_rsm_object* Object, dg_serial Serial ) {
	return dg_rsm_object_access_struct_via_cert(Object, dg_rsm_general_helper_serial_to_cert( Serial ));
}

//If optimizing do not use this, just pack simd instructions of the i16 to check for nulls and shit.
static inline bool dg_rsm_object_helper_contains_cert( dg_rsm_object* Object, dg_canon Cert) {
	return !(dg_rsm_object_helper_retrieve_cert_index(Object, Cert) < 0);
}

static inline dg_canon dg_rsm_object_helper_raw_cert_at( dg_rsm_object* Object, dg_u8 Index ) {
	return dg_rsm_object_access_ptr_certs( Object )[Index];
}

#define DG_RSM_GET( Type, Obj, Cert ) \
	((Type*)dg_rsm_object_access_struct_via_cert((Obj),(Cert)))

static inline dg_rsm_object dg_rsm_object_helper_compute_simple_offsets( dg_u8 ItemCount, dg_inwords16 CertsOffset ) {
...
static inline dg_inbytes dg_rsm_object_helper_sum_of_bytes( dg_rsm_object* Object ) {
...
dg_errcode dg_rsm_registry_process_initialize( dg_rsm_registrar* OutRegistrar, dg_allocator* Alloc, dg_u32 RequestedCapacity ) {
	...
	*OutRegistrar =  (dg_rsm_registrar) {
	...
	// not the caller's Alloc pointer, so the pool is safe once this function returns.
...
dg_errcode dg_rsm_registry_process_enroll( dg_rsm_registrar* Registrar, dg_serial Serial,  dg_rsm_registry_accessor* OutAccessor );

dg_errcode dg_rsm_registry_process_disenroll( dg_rsm_registrar* Registrar, dg_serial Serial );

dg_errcode dg_rsm_registry_procedure_reacclimatize( dg_rsm_registrar* Registrar );

dg_errcode dg_rsm_registry_procedure_grow( dg_rsm_registrar* Registrar );

dg_errcode dg_rsm_registry_process_maintenance( dg_rsm_registrar* Registrar );
